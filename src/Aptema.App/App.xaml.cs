...
{
    private readonly Aptema.Infrastructure.StartupRegistrationService _startupRegistration = new();
    ...
    private IReadOnlyList<Aptema.Core.MonitorModel> _detectedMonitors = Array.Empty<Aptema.Core.MonitorModel>();
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        Aptema.Application.IBrightnessController brightnessController = noHardware || safeMode
        ...
        Aptema.Application.IContentLuminanceSampler contentLuminanceSampler = safeMode
    ...
    private async Task MarkStartupHealthyAfterDelayAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(executablePath)) _startupRegistration.MigrateLegacyRegistration(executablePath);
    }
    ...
    private void IdentifyDisplay(Aptema.Core.MonitorModel monitor)
    {
    ...
    private async Task TestDisplayAsync(Aptema.Core.MonitorModel monitor)
    {
        ...
        var baseline = _viewModel.RuntimeSnapshot.Displays.FirstOrDefault(item => item.Monitor.Id == monitor.Id)?.Decision
            ...
            ?? new Aptema.Core.ComfortDecision(Aptema.Core.ComfortProfileId.Auto, 60, 5200, 0, TimeSpan.FromSeconds(2), true, "Display check", []);
